...
import com.kelompokcool.entity.ChatMessage
...
class ChatActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Instantiate the toolbar to show the username of the chat user
        if (userID != null) {
            BaseFirebaseProperties.rootDB.collection("users").document(userID)
        ...
        val chatQuery =
            userID?.let { realtimeDB.reference.child(authDb.currentUser?.uid!!).child(it).child("messages").orderByChild("sendAt") }
        // Set our recyclerview to use LinearLayoutManager
        ...
        button_chat_send.setOnClickListener {
            if (edit_chat_message.text.isNotEmpty()) {
                val tempChatMessage = ChatMessage(
                    edit_chat_message.text.toString(),
                    getTime(),
                    authDb.currentUser?.uid
                )
                userID?.let {
                    realtimeDB.reference.child(authDb.currentUser?.uid!!).child(it)
                        .child("messages").push().setValue(tempChatMessage)
                }
                if (userID != null) {
                    realtimeDB.reference.child(userID).child(authDb.currentUser?.uid!!)
                        .child("messages").push().setValue(tempChatMessage)
                }
