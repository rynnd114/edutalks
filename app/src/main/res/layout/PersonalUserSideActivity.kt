...
import com.kelompokcool.entity.Post
...
class PersonalUserSideActivity : AppCompatActivity() {
        ...
        override fun onBindViewHolder(
            ...
            if (!displayStatus) {
                ...
                if(model.postIsPhoto!!) {
                    ...
                    Glide.with(this@PersonalUserSideActivity).load(BaseFirebaseProperties.imageRef.child("images/" + model.id + ".jpeg")).into(holder.itemView.ImageView_post);
                } else {
                    ...
                    try {
                        reviewed.forEach { (key, value) ->
                            if (value.contains(authDb.currentUser!!.uid) && key == review) {
                                ...
                                this@PersonalUserSideActivity.targetId = resources.getIdentifier(
                                    textViewTarget.toString(), "id",
                                    packageName
                                )
                                model.review!![review]?.size?.let {
                                    this@PersonalUserSideActivity.oldSize = model.review!![review]?.size!!
                                }
                            } else if (!value.contains(authDb.currentUser!!.uid) && key == review) {
                                value.add(authDb.currentUser!!.uid)
